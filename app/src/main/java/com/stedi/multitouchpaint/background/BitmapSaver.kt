...
class BitmapSaver(private val target: Bitmap) : Thread() {
    ...
    override fun run() {
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            ...
            if (dirAvailable) {
                val fileName = App.fileNamePrefix + System.currentTimeMillis() + ".png"
                ...
                try {
                    FileOutputStream(file).use {
                        if (target.compress(Bitmap.CompressFormat.PNG, 100, it)) {
                            ...
                            PendingRunnables.post(Runnable { App.getBus().post(Callback.BITMAP_SAVED) })
                        } else {
                            PendingRunnables.post(Runnable { App.getBus().post(Callback.FAILED_TO_SAVE) })
                        }
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                ...
                PendingRunnables.post(Runnable { App.getBus().post(Callback.CANT_SAVE) })
            }
        } else {
            PendingRunnables.post(Runnable { App.getBus().post(Callback.CANT_SAVE) })
        }
